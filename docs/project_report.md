...
Bạn có **2 backend** dùng chung một entrypoint:

#### Cách 1: Mock Hardware (không cần Gazebo)
Lệnh này tự khởi động Controller Manager ảo và mô phỏng chuyển động robot:
```bash
ros2 launch my_robot_arm_pick_place pick_place.launch.py backend:=mock
```

#### Cách 2: Gazebo Sim (một terminal)
Lệnh này tự khởi động Gazebo, robot, controllers, MoveIt và node Pick & Place:
```bash
ros2 launch my_robot_arm_pick_place pick_place.launch.py backend:=gazebo
```

Có thể thêm `headless:=true` để không mở GUI hoặc `autorun:=false` để chỉ kiểm tra TF, controllers và planning trước khi phát lệnh chuyển động.
